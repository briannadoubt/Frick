...
Own backend Frick work. Use the spine and public docs, avoid demo internals, preserve tenant isolation and structured errors, and regenerate schema artifacts instead of editing generated files. Use documented `@frick/server` surfaces, including `appRoutes`, `jobs.handlers`, `recurring.jobs`, policy hooks, and registry options; outbound email helpers under `apps/server/src/email/*` are internal until exported.
