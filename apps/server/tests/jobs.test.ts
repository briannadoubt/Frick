...
describe("JobStore enqueue/claim/complete", () => {
  it("round-trips a single job through enqueue → claim → complete", () => {
    ...
    const claimed = store.jobs.claim("worker-a");
    ...
    store.jobs.complete(row.id, { ok: true });
  ...
  it("returns the same row when re-enqueueing with the same idempotency key", () => {
    const store = newStore();
    const first = store.jobs.enqueue({
      tenantId: "_default",
      jobType: "PushNotification",
      payload: { x: 1 },
      idempotencyKey: "abc",
    });
    const second = store.jobs.enqueue({
      tenantId: "_default",
      jobType: "PushNotification",
      payload: { x: 2 },
      idempotencyKey: "abc",
  ...
  it("claim is atomic — two concurrent claim calls never see overlap", () => {
    const store = newStore();
    for (let i = 0; i < 10; i++) {
      store.jobs.enqueue({
        tenantId: "_default",
    ...
    const a = store.jobs.claim("worker-a", undefined, 6);
    const b = store.jobs.claim("worker-b", undefined, 6);
  ...
  it("retryable failure re-arms with exponential backoff", () => {
    const store = newStore();
    const row = store.jobs.enqueue({
      tenantId: "_default",
      jobType: "TestJob",
    ...
    const claimed = store.jobs.claim("worker-a")[0]!;
    expect(claimed.attemptCount).toBe(1);
    store.jobs.fail(claimed.id, "test.transient", "boom", true);
    const after = store.jobs.getById(row.id);
  ...
  it("non-retryable failure dead-letters immediately", () => {
    const store = newStore();
    const row = store.jobs.enqueue({
      tenantId: "_default",
      jobType: "TestJob",
    ...
    const claimed = store.jobs.claim("worker-a")[0]!;
    store.jobs.fail(claimed.id, "test.fatal", "no retry", false);
    const after = store.jobs.getById(row.id);
  ...
  it("dead-letters when max_attempts is reached", () => {
    const store = newStore();
    const row = store.jobs.enqueue({
      tenantId: "_default",
      jobType: "TestJob",
    ...
    // Attempt 1
    let claimed = store.jobs.claim("worker-a")[0]!;
    expect(claimed.attemptCount).toBe(1);
    store.jobs.fail(claimed.id, "test.retry", "first", true);
    expect(store.jobs.getById(row.id)?.status).toBe("ready");

    // Reset available_at so we can claim again without waiting for backoff.
    store.jobs.list({ status: "ready" }); // sanity
    ...
    const row2 = store.jobs.enqueue({
      tenantId: "_default",
      jobType: "TestJob",
      payload: {},
      maxAttempts: 1,
    });
    const claimed2 = store.jobs.claim("worker-a", "TestJob", 1)[0]!;
    expect(claimed2.attemptCount).toBe(1);
    store.jobs.fail(claimed2.id, "test.retry", "exhausted", true);
    const after = store.jobs.getById(row2.id);
    expect(after?.status).toBe("dead_lettered");
    store.close();
  });

  it("tenant isolation — claim with a tenant filter via list won't cross tenants", () => {
    const store = newStore();
    store.jobs.enqueue({ tenantId: "tenant-a", jobType: "T", payload: {} });
    store.jobs.enqueue({ tenantId: "tenant-b", jobType: "T", payload: {} });
    expect(store.jobs.list({ tenantId: "tenant-a" })).toHaveLength(1);
    expect(store.jobs.list({ tenantId: "tenant-b" })).toHaveLength(1);
    expect(store.jobs.list({ tenantId: "tenant-a" })[0]!.tenantId).toBe("tenant-a");
    // getById with a tenant filter only finds within that tenant.
    const aRow = store.jobs.list({ tenantId: "tenant-a" })[0]!;
    expect(store.jobs.getById(aRow.id, "tenant-b")).toBeUndefined();
    expect(store.jobs.getById(aRow.id, "tenant-a")?.id).toBe(aRow.id);
    store.close();
  });

  it("countsByStatus exposes a per-state snapshot", () => {
    const store = newStore();
    const a = store.jobs.enqueue({ tenantId: "_default", jobType: "T", payload: {} });
    const b = store.jobs.enqueue({ tenantId: "_default", jobType: "T", payload: {} });
    store.jobs.enqueue({ tenantId: "_default", jobType: "T", payload: {} });
    const [first, second] = store.jobs.claim("worker-a", "T", 2);
    store.jobs.complete(first!.id);
    store.jobs.fail(second!.id, "test.fatal", "x", false);
    const counts = store.jobs.countsByStatus();
...
describe("jobBackoffMs", () => {
  it("doubles per attempt and caps at 5 minutes", () => {
