...
- **`@frick/server`:** A sharing grant on an object record now cascades READ access to the derived primitives keyed by that record's id, within the same tenant: the stream whose `streamId` equals the granted record id, and the projection rows whose subscribe/read `key` equals that id (FR-70). The cascade runs after the baseline policy + app hooks + object-record grant relaxation and only ever flips a deny to allow (never the reverse); it is strictly read-only (`stream.append` and writes are never relaxed), tenant-scoped, and fails closed when there is no resolvable row id (e.g. a whole-projection subscribe with no `key`). Revoking the grant restores the underlying deny. New `assertCanSubscribe` cascade parameter backed by `FrickCascadeGrantLookup` / `relaxWithCascadeGrants` in authz and `GrantStore.hasActiveGrantForRecordId`. See [`docs/threat-model.md`](docs/threat-model.md).

### Server — encrypted Web Push payloads (RFC 8291)

- **`@frick/server`:** The Web Push adapter now encrypts notification content per RFC 8291 (Message Encryption for Web Push) using the `aes128gcm` content encoding (RFC 8188), so the title/body/data reach the browser Service Worker instead of an empty wake-up (FR-60). When a `PushSubscription` registration carries the browser's `keys.p256dh` + `keys.auth`, the adapter generates an ephemeral P-256 keypair, derives the shared secret via ECDH, runs the RFC 8291 / RFC 8188 HKDF key + nonce derivation, AES-128-GCM encrypts a single record, and frames the body as `salt(16) || rs(4) || idlen(1) || keyid` + ciphertext with `Content-Encoding: aes128gcm` and `Content-Length` alongside the existing VAPID `Authorization` header. A fresh ephemeral key and salt are used per send; an over-4096-octet body returns `push.payloadTooLarge` without dispatch. Registrations without subscription keys keep the backward-compatible empty-body path. New export `encryptWebPushPayload` from `@frick/server/push/web-push-adapter`. Multi-key rotation of the encryption material (FR-61) and APNs/FCM are unchanged. See [`docs/push-adapters.md`](docs/push-adapters.md).
