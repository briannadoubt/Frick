...
- **Platform event pipeline baseline** — the server now ships a durable SQLite platform event pipeline, a KafkaJS Redpanda/Kafka adapter boundary, backup/restore preservation, and authenticated health at `/_frick/inspect/platform-events` plus `/_frick/dashboard/api/platform-events/health`.
