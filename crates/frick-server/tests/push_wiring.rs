...
fn recording_seams(apns: Arc<RecordingApnsTransport>) -> BootSeams {
    BootSeams {
        ...
        // FR-307: no connection-lifecycle hooks for the push wiring tests.
        connection_lifecycle: Vec::new(),
        // AURA-323: no federation hooks for the push wiring tests.
...
async fn boot_registers_push_deliver_handler() {
    ...
    // purpose-limited PushKit, the other two live adapters, plus `test`).
    for platform in [
        PushPlatform::Apns,
